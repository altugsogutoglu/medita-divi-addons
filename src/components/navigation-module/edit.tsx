...
// Local Dependencies.
import { NavigationModuleEditProps } from './types';
 ...
 */
const NavigationModuleEdit = (props: NavigationModuleEditProps): ReactElement => {
  const {
    attrs,
    id,
    name,
        ...
        ! isLoading && (
          <>
            <ElementComponents
              ...
              id={id}
            />
            <div className="example_navigation_module__inner">
              {elements.render({
                attrName: 'title',
              })}
              <div className="example_navigation_module__post-items">
                {
                  map(response, (post) => (
                      <div className="example_navigation_module__post-item" key={post?.id}>
                        <PostTitleHeading className="example_navigation_module__post-item-title">
                          <a href={post?.link} onClick={() => false}>{post?.title?.rendered}</a>
                        </PostTitleHeading>
                        <div className="example_navigation_module__post-item-content" dangerouslySetInnerHTML={{__html: post?.excerpt?.rendered}} />
                      </div>
                    )
                  )
                }
